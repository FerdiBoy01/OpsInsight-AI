...
export default function Dashboard({ alerts, showAnalytics = true }) {
  ...
  const renderTourPointer = () => {
    switch (tourStep) {
      ...
      case 8: return <div className="absolute top-[50%] -translate-y-1/2 -right-2 w-4 h-4 bg-white dark:bg-[#121214] rotate-45 border-r border-t border-slate-200 dark:border-zinc-800"></div>; 
      default: return null;
    }
  };
  ...
  // ==========================================
  // STATE & FUNGSI GENERATIVE AI GEMINI (DSS) 🔥
  // ==========================================
  ...
  return (
    <main className="px-8 pb-6 h-full flex flex-col transition-colors duration-500 relative overflow-y-auto custom-scrollbar">
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[9px] font-black uppercase tracking-widest text-slate-600 dark:text-zinc-300">
          <span className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-500/10 text-blue-600 border border-blue-200 dark:border-blue-500/20 px-3 py-1.5 rounded-lg shadow-sm">
            <BrainCircuit size={12} /> AI Detection
          </span>
          <span className="text-slate-300 dark:text-zinc-700">•</span>
          <span className="flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 border border-emerald-200 dark:border-emerald-500/20 px-3 py-1.5 rounded-lg shadow-sm">
            <Zap size={12} /> Real-time Monitoring
          </span>
          <span className="text-slate-300 dark:text-zinc-700">•</span>
          <span className="flex items-center gap-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 border border-indigo-200 dark:border-indigo-500/20 px-3 py-1.5 rounded-lg shadow-sm">
            <BarChart3 size={12} /> Analytics Dashboard
          </span>
          <span className="text-slate-300 dark:text-zinc-700">•</span>
          <span className="flex items-center gap-1.5 bg-purple-50 dark:bg-purple-500/10 text-purple-600 border border-purple-200 dark:border-purple-500/20 px-3 py-1.5 rounded-lg shadow-sm">
            <Bot size={12} /> Decision Support
          </span>
        </div>
      </div>
      ...
      {showTourPrompt && tourStep === 0 && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[9999] bg-blue-600/80 backdrop-blur-xl p-5 rounded-3xl shadow-[0_20px_50px_rgba(37,99,235,0.4)] border border-blue-400/30 w-[360px] animate-in slide-in-from-top-[-50px] fade-in duration-500 transition-all">
      ...
      {/* 1. KARTU STATISTIK */}
      <div className={`grid grid-cols-4 gap-4 mb-4 flex-shrink-0 transition-all duration-500 ${tourStep === 1 ? 'relative z-[10001]' : 'relative z-10'}`}>
        {[
          ...
          <div key={i} className={`bg-white dark:bg-[#121214] p-4 rounded-2xl border border-slate-200 dark:border-zinc-800/60 flex flex-col shadow-sm transition-all group hover:border-blue-500/50 ${tourStep === 1 ? 'ring-4 ring-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.3)] bg-slate-50 dark:bg-[#09090b] scale-[1.01] relative z-[10002]' : 'relative z-10'}`}>
            <div className="flex items-center justify-between mb-2 relative z-10">
              <p className="text-slate-500 dark:text-zinc-500 text-[10px] font-bold uppercase tracking-widest leading-none">{item.label}</p>
              <span className="text-[7px] font-black uppercase tracking-widest text-slate-400 bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded">AI Generated</span>
            </div>
            <div className="flex items-center gap-4 mb-2 relative z-10">
              <div className={`p-2.5 rounded-xl ${item.bg}`}><item.icon className={item.color} size={20} /></div>
              <div>
                <div className="flex items-baseline gap-1 mt-1">
                  <h3 className="text-2xl font-black text-slate-900 dark:text-zinc-100 leading-none">{item.val}</h3>
                  <span className="text-[10px] font-bold text-slate-400 dark:text-zinc-600 uppercase tracking-tighter">{item.unit}</span>
                </div>
              </div>
            </div>
            <p className="text-[9px] text-slate-400 dark:text-zinc-500 italic font-medium mt-2 relative z-10">{item.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-12 gap-5 flex-1 min-h-0">
        ...
        <div className={`col-span-8 flex flex-col gap-4 min-h-0 transition-all duration-500 ${tourStep >= 2 && tourStep <= 6 ? 'relative z-[10001]' : ''}`}>
              ...
              <div className="flex items-center gap-3">
                {!isFullscreen && (
                  <>
                    ...
                    >
                      {isAiActive ? <BrainCircuit size={12} /> : <PowerOff size={12} />} {isAiActive ? 'Analisis AI On' : 'Analisis AI Off'}
                    </button>
                    
                    <div className={`relative ${tourStep === 3 ? 'ring-4 ring-rose-500 ring-offset-2 dark:ring-offset-[#09090b] animate-pulse scale-110 z-50 rounded-lg shadow-lg shadow-rose-500/50' : ''}`}>
                      <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                      <select onChange={handleSwitchCamera} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-zinc-300 text-[10px] font-bold uppercase rounded-lg py-1.5 pl-2.5 pr-8 focus:outline-none cursor-pointer appearance-none">
                ...
                <button onClick={() => { if(!document.fullscreenElement) videoContainerRef.current.requestFullscreen(); else document.exitFullscreen(); }} className="p-1.5 hover:bg-slate-200 dark:hover:bg-zinc-800 rounded-lg text-slate-500 transition-colors">
                   <Maximize size={14}/>
                </button>
              </div>
            </div>
            
            <div className="bg-black relative flex items-center justify-center overflow-hidden flex-1 min-h-[300px] relative z-10">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2 z-10 pointer-events-none uppercase tracking-widest text-[9px] font-bold text-white bg-black/60 px-3 py-1.5 rounded-lg border border-white/10 backdrop-blur-sm">
                   Tracking: {isAiActive ? 'PPE Detection On' : 'Standby'}
              </div>
              <div className="absolute bottom-4 right-4 bg-black/60 px-3 py-1.5 rounded-lg text-white font-mono text-[11px] font-bold tracking-widest backdrop-blur-sm border border-white/10 z-10">
          ...
          {showAnalytics && (
            <div className="grid grid-cols-2 gap-4 h-[160px] flex-shrink-0">
              {/* GRAFIK 24 JAM */}
              <div className={`bg-white dark:bg-[#121214] p-4 rounded-2xl border border-slate-200 dark:border-zinc-800/60 shadow-sm flex flex-col min-h-0 transition-all duration-500 ${tourStep === 5 ? 'ring-4 ring-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.3)] bg-slate-50 dark:bg-[#09090b] scale-[1.01] relative z-[10002]' : 'relative z-10'}`}>
              ...
              {/* GRAFIK WAKTU */}
              <div className={`bg-white dark:bg-[#121214] p-4 rounded-2xl border border-slate-200 dark:border-zinc-800/60 shadow-sm flex flex-col min-h-0 transition-all duration-500 ${tourStep === 6 ? 'ring-4 ring-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.3)] bg-slate-50 dark:bg-[#09090b] scale-[1.01] relative z-[10002]' : 'relative z-10'}`}>
                <div className="flex justify-between items-center mb-3 relative z-10">
        ...
        {/* KOLOM KANAN (DSS & LOG) */}
        <div className={`col-span-4 flex flex-col gap-4 min-h-0 transition-all duration-500 ${tourStep >= 7 ? 'relative z-[10001]' : ''}`}>
          
          {/* REKOMENDASI DSS */}
          <div className={`bg-blue-600 text-white rounded-2xl p-5 shadow-lg flex-shrink-0 relative overflow-hidden transition-all duration-500 ${tourStep === 7 ? 'ring-4 ring-blue-400 shadow-[0_0_40px_rgba(59,130,246,0.5)] scale-[1.02] relative z-[10002]' : 'relative z-10 shadow-blue-500/20'}`}>
             ...
             <div className="flex justify-between items-start mb-4 relative z-10">
               <div>
                 <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-1">
                   <BrainCircuit size={14} className="text-blue-200" /> AI DECISION SUPPORT SYSTEM
                 </h3>
               ...
               <button 
                 ...
                 className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 active:scale-95 transition-all text-[8px] font-black uppercase tracking-widest px-2.5 py-1.5 rounded-lg border border-white/20 disabled:opacity-50 cursor-pointer"
          ...
          {/* LOG BAHAYA */}
          <div className={`bg-white dark:bg-[#121214] rounded-2xl border border-slate-200 dark:border-zinc-800/60 flex flex-col flex-1 min-h-0 shadow-sm overflow-hidden transition-all duration-500 ${tourStep === 8 ? 'ring-4 ring-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.3)] scale-[1.01] relative z-[10002]' : 'relative z-10'}`}>
