...
CAMERA_API_URL = 'http://localhost:3000/api/cameras/active' 
CONFIG_API_URL = 'http://localhost:3000/api/config' # <--- ENDPOINT SAKLAR AI BARU
...
def get_system_config():
    cam_url = 'simulasi4.mp4'
    ai_active = True # Default nyala
...
def generate_frames():
    ...
    while True:
        ...
        # Cek Database tiap 30 frame (~1 detik) biar kalau tombol diklik, langsung bereaksi!
        if frame_counter % 30 == 0:
            new_url, new_ai_active = get_system_config()
            ai_active = new_ai_active # Update status saklar AI
        ...
        # ==========================================
        # LOGIKA SAKLAR AI (KUNCI PERUBAHANNYA DI SINI)
        # ==========================================
        if ai_active:
            # JIKA AI ON -> JALANKAN YOLO DAN BIKIN KOTAK
                ...
                for (vx1, vy1, vx2, vy2, v_name, v_conf) in violation_boxes:
                    ...
                    if not is_false_alarm:
                        ...
                        if current_time - last_alert_time.get(v_label, 0) > cooldown_time:
                            try:
                                snap_resized = cv2.resize(frame, (640, 360))
                                _, snap_buffer = cv2.imencode('.jpg', snap_resized, [int(cv2.IMWRITE_JPEG_QUALITY), 50])
            ...
            # JIKA AI MATI -> MODE NORMAL (Skip YOLO, langsung lewati)
            pass
